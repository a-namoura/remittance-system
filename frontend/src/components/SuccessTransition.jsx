...
export default function SuccessTransition({ message, playSound = true }) {
  useEffect(() => {
    if (!message || !playSound) return;
    playSuccessSound();
  }, [message, playSound]);
    ...
    <div className="fixed inset-0 z-[90] flex items-center justify-center bg-white/55 px-4 backdrop-blur-sm dark:bg-gray-950/70">
      ...
      >
        <div className="app-success-illustration relative mx-auto h-20 w-20">
          <span className="app-success-pulse absolute inset-1 rounded-full bg-green-200/70 dark:bg-green-500/25" />
          ...
          <svg
            viewBox="0 0 96 96"
            className="relative h-20 w-20 text-green-700 dark:text-green-300"
            fill="none"
            stroke="currentColor"
            strokeLinecap="round"
        ...
        <p className="mt-3 text-base font-semibold text-green-700 dark:text-green-300">
          {message}
        </p>
      </div>
    </div>
