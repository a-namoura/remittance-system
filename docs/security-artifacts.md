...
The `Dependency inventory` CI job generates an inventory of direct backend, frontend, and contracts dependencies from the checked-out manifests and lockfiles. Each workflow run retains the generated JSON artifact for 90 days, so dependency updates cannot leave a committed derived snapshot stale. Release workflows generate the same inventory from the exact release ref and include it with the archived security evidence.
