...
The restore command decrypts and authenticates the `RMBK1` archive before piping it only to `mongorestore`; it intentionally omits `--drop`, so use an empty target database. Immediately afterward it receipt-reconciles every restored `txHash` using `BSC_TESTNET_RPC_URL` and `REM_CONTRACT_ADDRESS`, without event ingestion or a signer. The command fails unless every hash received a reconciliation check with no receipt/RPC errors. To repeat only this read-only step after a completed restore, run `RESTORE_CONFIRM=receipt-only npm run restore:reconcile`. The integrity check then confirms unique transaction hashes plus payment-link-to-transaction hash and transaction-to-payment-link references. Treat any failure as a failed restore: discard the isolated target and investigate the archive/key before retrying. Record the archive identifier, target database, reconciliation and check output, and operator in the recovery ticket before making the database available to the application.
